---
name: 'step-04a-discord-setup'
description: '디스코드 봇 생성, 토큰 발급, 환경변수 설정, 플러그인 설치/설정 → 세션 종료 안내'

nextStepFile: './step-04b-discord-connect.md' # 세션 종료로 직접 사용되지 않음 — step-01b가 라우팅
...
Discord Developer Portal에서 봇을 생성하고, 토큰을 발급받고, 환경변수 설정 + Claude Channels 플러그인 설치/설정까지 완료합니다. 환경변수 적용 및 `--channels` 플래그 실행을 위해 세션을 종료하고 `/resume`으로 돌아오도록 안내합니다.
...
- 🎯 봇 생성, 토큰 발급, 환경변수 설정, 플러그인 설치/설정에 집중하세요
- 🚫 프사 설정, 페어링, discord.py 스크립트는 step-04b에서 합니다
...
- 🚫 환경변수 설정 및 플러그인 설정 전에 다음 단계로 넘어가지 마세요
...
`{statusFile}`의 `stepsCompleted`에 `step-04a-discord-setup`을 추가합니다.
...
다시 돌아오면 디스코드 페어링, 스크립트 세팅, 프사 설정을 할 거예요 🎉"
...
- 봇 토큰 발급 완료
- 봇이 서버에 초대됨
- 환경변수 설정 완료 (discord.py용)
- 디스코드 플러그인 설치 완료
- `/discord:configure` 토큰 등록 완료
- 상태 파일 업데이트됨 (step-04b로 라우팅 가능)
