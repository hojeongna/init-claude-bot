...
description: '디스코드 봇 생성, 토큰 발급, 환경변수 설정 → 세션 종료 안내'
...
Discord Developer Portal에서 봇을 생성하고, 토큰을 발급받고, 환경변수를 설정합니다. 환경변수 적용을 위해 세션을 종료하고 `/resume`으로 돌아오도록 안내합니다.
...
- 🎯 봇 생성, 토큰 발급, 환경변수 설정에만 집중하세요
- 🚫 플러그인 설치나 프사 설정은 step-04b에서 합니다
- 💬 각 단계를 하나씩 안내하고, 사용자가 따라왔는지 확인하세요
- ⚠️ 이 스텝 끝에서 반드시 세션 종료 + `/resume` 안내를 하세요
...
다시 돌아오면 디스코드 플러그인 설치와 프사 설정을 할 거예요 🎉"
...
**Master Rule:** 봇 토큰은 절대 채팅에 노출되면 안 됩니다. 환경변수로만 설정하세요.
