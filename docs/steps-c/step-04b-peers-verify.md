...
- `--dangerously-skip-permissions` — 크론 세션은 자율적으로 파일 수정, 스크립트 실행을 해야 하므로 필요해요
- `--dangerously-load-development-channels server:claude-peers` — peers 간 메시지가 즉시 전달돼요 (없으면 수동 폴링 필요)
